...
from domainSpecific.BaseClient import BaseClient
...
class PeopleClient(BaseClient):
    ...
    def create_person(self,body=None):
        last_name,response = self.__created_person_with_unique_last_name(body)
        return last_name,response

    def __created_person_with_unique_last_name(self, body=None):
        if body is None:
            last_name = f'User {str(uuid4())}'
            payload = dumps({
                'fname':'JaiShriKrishna',
                'lname': last_name
                })
        else:
            last_name = body['lname']        
            payload = dumps(body)
        print (" Value of ", self.request)
